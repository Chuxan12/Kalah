...
const GameBoard = () => {
  const [boardState, setBoardState] = useState({
    holes: new Map([
      [0, 8], [1, 7], [2, 4], [3, 3], [4, 4], [5, 4], [6, 4],
      [7, 2], [8, 12], [9, 4], [10, 4], [11, 7]
    ]),
    kalahs: new Map([[0, 10], [1, 0]]),
    selectedHole: null, 
  });
  ...
  useEffect(() => {
    async function getGameState() {
      axios.get('api/get_game') // получаем состояние игры
        .then((response) => {
  ...
  const handleFinishTurn = async () => { // по нажатии завершить ход отправляем ид лунки и ждем ответ
    try {
      const response = await axios.post('/api/turn_confirm', { holeId: boardState.selectedHole });
      const data = response.data;

      const holes = data.holes;
      const kalahs = data.kalahs;

      setBoardState((prevState) => ({ ...prevState, holes, kalahs, selectedHole: null }));
